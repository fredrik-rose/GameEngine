...
sudo apt install -y ccache strace

sudo apt install -y pkg-config unzip zip
...
only option is to build from source (typically the case if using WSL).
...
usual but append `--graphviz=dependency_graph.dot` to the command. An image of the graph can then
